<x-app-layout title="{{ $title ?? 'Ubah Mapping'}}">
    <section class="section">
        <div class="section-header">
            ...
            <div class="section-header-breadcrumb">
                ...
                <div class="breadcrumb-item">Ubah Mapping</div>
            </div>
        </div>

        <div class="section-body">
            <h2 class="section-title">Ubah Mapping</h2>
            <p class="section-lead">
                Silahkan isi semua kolom berikut untuk memperbarui Mapping.
            </p>
            <div class="row">
                <div class="col-12">
                    <div class="card">
                        ...
                        <form action="{{ route('encounter.mapping.update', $mapping->id)}}" method="POST">
                            @csrf
                            @method('put')
                            <div class="card-body">
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select id="practitioner_id" class="form-control select2" name="practitioner_id">
                                            ...
                                            <option value="{{ $id }}" {{ $id == $mapping->dokter_id ? 'selected' : ''}}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select id="location_id" class="form-control select2" name="location_id">
                                            ...
                                            <option value="{{ $id }}" {{ $id == $mapping->location_id ? 'selected' : ''}}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
                                <div class="form-group row mb-4">
                                    ...
                                    <div class="col-sm-12 col-md-7">
                                        <select id="cara_masuk" class="form-control selectric" name="cara_masuk">
                                            @foreach($pelayanans as $id => $name)
                                            <option value="{{ $id }}" {{ $id == $mapping->cara_masuk ? 'selected' : ''}}>{{ $name }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                </div>
